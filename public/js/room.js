...
//whiteboard js end
// 🧑‍🤝‍🧑 Attendees DOM update logic
let attendeeList = {};
function updateAttendeesDOM() {
    const listContainer = document.querySelector('#attendees-ul');
    if (!listContainer) {
        console.error("Element with ID 'attendees-ul' not found in DOM.");
        return;
    }

    listContainer.innerHTML = '';

    for (const [sid, uname] of Object.entries(attendeeList)) {
        const li = document.createElement('li');
        li.textContent = (sid === socket.id) ? `${uname} (You)` : uname;
...
document.getElementById('attendees-tab').addEventListener('click', () => {
    document.getElementById('chat-section').style.display = 'none';
    document.getElementById('attendees-section').style.display = 'block';
    setActiveTab('attendees-tab');
});
...
socket.on('attendees-update', (userMap) => {
    attendeeList = userMap;
    updateAttendeesDOM();
});
